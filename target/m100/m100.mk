# === TRS-80 Model 100 ===

M100ZCCOPTS := +m100 -subtype=default

m100 :

	mkdir $@

.PHONY : m100-hw
m100-hw : target/m100/loader.ba m100/hw.hex | $(ORTER)

	@$(PROMPT) 'On the target type RUN "COM:78N1E" <enter>'
	@printf '* \033[1;33mLoading loader\033[0;0m\n'
	(while read -r l; do echo "$$l"; sleep 1; done < target/m100/loader.ba && printf '\032' && sleep 1) | $(ORTER) serial -o ixon -o ixoff -o onlcrx -e 2 $(SERIALPORT) 4800
	@printf '* \033[1;33mLoading hex\033[0;0m\n'
	(while read -r l; do echo "$$l"; sleep 2; done < m100/hw.hex && printf '\032' && sleep 1) | $(ORTER) serial -o ixon -o ixoff -o onlcrx -e 2 $(SERIALPORT) 4800

m100/hw.co : hw.c | m100

	zcc $(M100ZCCOPTS) \
		-create-app -m -o $@ $<

m100/hw.hex : m100/hw.co

	xxd -p -u < $< > $@ 

m100/inst.co : main.c m100/rf.lib m100/system.lib m100/inst.lib

	zcc $(M100ZCCOPTS) \
		-lm100/rf -lm100/system -lm100/inst \
		-create-app -m -o $@ $<

m100/inst.lib : inst.c inst.h rf.h target/m100/m100.inc | m100

	zcc $(M100ZCCOPTS) -x -o $@ $<

m100/rf.lib : rf.c rf.h target/m100/m100.inc | m100

	zcc $(M100ZCCOPTS) -x -o $@ $<

m100/system.lib : target/m100/system.c rf.h target/m100/m100.inc | m100

	zcc $(M100ZCCOPTS) -x -o $@ $<
